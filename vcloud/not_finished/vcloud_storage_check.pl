...
sub check_vapp_settings($$) {
  ...
  foreach my $NetworkName ( @{$content_page_ref->{VApp}[0]{Children}[0]{Vm}[0]{ovf:Item}} ) {
#    my $nName = $NetworkName->{networkName};
#    my $nState = $NetworkName->{Configuration}[0]{Features}[0]{FirewallService}[0]{IsEnabled}[0];

my $nName = $NetworkName->{'rasd:ElementName'};

print "$NetworkName $nName\n";
#    if (!defined $nState) {
#      $nState="";
#    } 
